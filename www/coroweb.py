...
def get_required_kw_args(fn):
	...
	for name,param in params.items():
		if param.kind==insepect.Parameter.KEYWORD_ONLY and param.default==inspect.Parameter.empty:
			args.append(name)
	return tuple(args)
...
class RequestHandler(object):
	...
	def __init__(self, app,fn):
		...
		self._named_kw_args=get_named_kw_argsfn(fn)
			...
			if request.method=='POST':
				...
				if ct.startswith('application/json'):
					...
					if not isinstance(params,dict):
						return web.HTTPBadRequest('jSON body must be obeject.')
					kw=params
